...
pub enum Event {
    ...
    MouseEnter             { mouse: DeviceID, window: WindowHandle, instant: EventInstant, is_grabbed: bool,  is_focused: bool, },
    MouseLeave             { mouse: DeviceID, window: WindowHandle, instant: EventInstant, was_grabbed: bool, was_focused: bool, },
    MouseButtonPressed     { mouse: DeviceID, window: WindowHandle, instant: EventInstant, button: MouseButton, clicks: Option<u32>, },
    MouseButtonReleased    { mouse: DeviceID, window: WindowHandle, instant: EventInstant, button: MouseButton, },
    MouseScroll            { mouse: DeviceID, window: WindowHandle, instant: EventInstant, scroll: Vec2<f64>, },
    ...
    KeyboardTextChar       { keyboard: DeviceID, window: WindowHandle, instant: EventInstant, char: char, is_repeat: bool, repeat_count: u32, },
    KeyboardTextString     { keyboard: DeviceID, window: WindowHandle, instant: EventInstant, text: String, is_repeat: bool, repeat_count: u32, },
    KeyboardKeyPressed     { keyboard: DeviceID, window: WindowHandle, instant: EventInstant, key: Key, is_repeat: bool, repeat_count: u32, },
    ...
    TabletStylusButtonPressed     { tablet: DeviceID, instant: EventInstant, window: WindowHandle, button: TabletStylusButton, },
    TabletStylusButtonReleased    { tablet: DeviceID, instant: EventInstant, window: WindowHandle, button: TabletStylusButton, },
    TabletStylusToolType          { tablet: DeviceID, instant: EventInstant, window: WindowHandle, tool_type: TabletStylusToolType, },
    TabletStylusMotion            { tablet: DeviceID, instant: EventInstant, window: WindowHandle, position: Vec2<f64>, root_position: Vec2<f64>, physical_position: Vec2<f64>, },
    TabletStylusPressure          { tablet: DeviceID, instant: EventInstant, window: WindowHandle, pressure: f64, },
    TabletStylusTilt              { tablet: DeviceID, instant: EventInstant, window: WindowHandle, tilt: Vec2<f64>, },
    ...
    TabletStylusButtonPressedRaw  { tablet: DeviceID, instant: EventInstant, button: TabletStylusButton, },
    TabletStylusButtonReleasedRaw { tablet: DeviceID, instant: EventInstant, button: TabletStylusButton, },
    TabletStylusToolTypeRaw       { tablet: DeviceID, instant: EventInstant, tool_type: TabletStylusToolType, },
    TabletStylusMotionRaw         { tablet: DeviceID, instant: EventInstant, physical_position: Vec2<f64>, },
    TabletStylusPressureRaw       { tablet: DeviceID, instant: EventInstant, pressure: f64, },
    TabletStylusTiltRaw           { tablet: DeviceID, instant: EventInstant, tilt: Vec2<f64>, },
...
impl Event {
    ...
    pub fn instant(&self) -> Option<EventInstant> {
        match *self {
            ...
            Event::MouseEnter             { mouse: _, instant, window: _, is_grabbed: _,  is_focused: _, } => Some(instant),
            Event::MouseLeave             { mouse: _, instant, window: _, was_grabbed: _, was_focused: _, } => Some(instant),
            Event::MouseMotion            { mouse: _, instant, window: _, position: _, root_position: _, } => Some(instant),
            Event::MouseButtonPressed     { mouse: _, instant, window: _, button: _, clicks: _, } => Some(instant),
            Event::MouseButtonReleased    { mouse: _, instant, window: _, button: _, } => Some(instant),
            Event::MouseScroll            { mouse: _, instant, window: _, scroll: _, } => Some(instant),
            ...
            Event::KeyboardTextChar       { keyboard: _, window: _, instant, char: _, is_repeat: _, repeat_count: _, } => Some(instant),
            Event::KeyboardTextString     { keyboard: _, window: _, instant, text: _, is_repeat: _, repeat_count: _, } => Some(instant),
            Event::KeyboardKeyPressed     { keyboard: _, window: _, instant, key: _, is_repeat: _, repeat_count: _, } => Some(instant),
            ...
            Event::TabletStylusButtonPressed     { tablet: _, instant, window: _, button: _, } => Some(instant),
            Event::TabletStylusButtonReleased    { tablet: _, instant, window: _, button: _, } => Some(instant),
            Event::TabletStylusToolType          { tablet: _, instant, window: _, tool_type: _, } => Some(instant),
            Event::TabletStylusMotion            { tablet: _, instant, window: _, position: _, root_position: _, physical_position: _, } => Some(instant),
            Event::TabletStylusPressure          { tablet: _, instant, window: _, pressure: _ } => Some(instant),
            Event::TabletStylusTilt              { tablet: _, instant, window: _, tilt: _, } => Some(instant),
            ...
            Event::TabletStylusButtonPressedRaw  { tablet: _, instant, button: _, } => Some(instant),
            Event::TabletStylusButtonReleasedRaw { tablet: _, instant, button: _, } => Some(instant),
            Event::TabletStylusToolTypeRaw       { tablet: _, instant, tool_type: _, } => Some(instant),
            Event::TabletStylusMotionRaw         { tablet: _, instant, physical_position: _, } => Some(instant),
            Event::TabletStylusPressureRaw       { tablet: _, instant, pressure: _ } => Some(instant),
            Event::TabletStylusTiltRaw           { tablet: _, instant, tilt: _, } => Some(instant),
